DROP TABLE IF EXISTS tesi.charlson; CREATE TABLE tesi.charlson AS
/* ------------------------------------------------------------------ */ 
...
WITH diag AS (
  ...
  FROM tesi.cohort AS ad
  LEFT JOIN diag
    ON ad.hadm_id = diag.hadm_id
	/*AGGIUNTA CLAUSOLA
left join mimiciv_icu.icustays as i on ad.hadm_id = i.hadm_id
WHERE ad.admittime< i.intime*/
GROUP BY ad.hadm_id
)
--select * from com;
, ag AS (
...
select * from tesi.charlson; --where age_score is null;